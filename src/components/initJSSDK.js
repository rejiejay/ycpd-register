...
let getWxConfig = function getWxConfig() {
    return new Promise((resolve, reject) => {
        $.ajax({
            url: `http://picc.hotgz.com/wx/apiHandler.ashx?action=WxConfig&url=${window.navigator.userAgent.match(/\(i[^;]+;( U;)? CPU.+Mac OS X/) ? encodeURIComponent(window.location.href.split('#')[0]) : encodeURIComponent(window.location.href)}`,
            type: "get",
            success(wxConfig) {
                resolve(wxConfig)
            },
